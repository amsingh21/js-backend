...
const changeCurrentPassword = asyncHandler(async(req,res)=>{
    ...
    // }
    const user = User.findById(req.user?._id) //  finding user based on user id 
     const isPasswordCorrect = await  user.isPasswordCorrect(oldPassword)
   // since isPasswordCorrect is async method so we have to put await 
     if(!isPasswordCorrect){
      throw new ApiError(400, "Invalid old password")
     }

     user.password = newPassword
     //  hook will be called when we save and 
     await user.save({validateBeforeSave:false})
 ...
 const getCurrentUser= asyncHandler(async(req,res)=>{
  return res
  .status(200)
  .json(200,req.user, "currenr user fetched sucessfully ")
 })
 ...
 const updateUserAvatar = asyncHandler(async(req,res)=>
{
  ...
  if (!avatarLocalPath) 
   ...
   const oldImageToBeDeleted=   avatarLocalPath 
   ...
   return res
     ...
     await deletOnCloudinary(avatarLocalPath)    })
